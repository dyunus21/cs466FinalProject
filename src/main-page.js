...
const MainPage = () => {
  ...
  const handleCellHover = (event, rowIndex, colIndex) => {
    let leftCell = document.querySelector(
      `.gridcell[data-row="${rowIndex}"][data-col="${colIndex - 1}"]`
    );
    if (leftCell) {
      leftCell.classList.add("gridcell-neighbor");
      leftCell.classList.remove("gridcell");
    } else {
      leftCell = document.querySelector(
        `.gridcell-optimal[data-row="${rowIndex}"][data-col="${colIndex - 1}"]`
      );
      if (leftCell) {
    ...
    let aboveCell = document.querySelector(
      `.gridcell[data-row="${rowIndex - 1}"][data-col="${colIndex}"]`
    );
    if (aboveCell) {
      aboveCell.classList.add("gridcell-neighbor");
      aboveCell.classList.remove("gridcell");
    } else {
      aboveCell = document.querySelector(
        `.gridcell-optimal[data-row="${rowIndex - 1}"][data-col="${colIndex}"]`
      );
      if (aboveCell) {
    ...
    let diagonalCell = document.querySelector(
      `.gridcell[data-row="${rowIndex - 1}"][data-col="${colIndex - 1}"]`
    );
    if (diagonalCell) {
      diagonalCell.classList.add("gridcell-neighbor");
      diagonalCell.classList.remove("gridcell");
    } else {
      diagonalCell = document.querySelector(
        `.gridcell-optimal[data-row="${rowIndex - 1}"][data-col="${
          colIndex - 1
        }"]`
      );
      if (diagonalCell) {
  ...
  const handleCellExit = (event, rowIndex, colIndex) => {
    let leftCell = document.querySelector(
      `.gridcell-neighbor[data-row="${rowIndex}"][data-col="${colIndex - 1}"]`
    );
    if (leftCell) {
      ...
      leftCell = document.querySelector(
        `.gridcell-optimal-neighbor[data-row="${rowIndex}"][data-col="${
          colIndex - 1
        }"]`
      );
      if (leftCell) {
    ...
    let aboveCell = document.querySelector(
      `.gridcell-neighbor[data-row="${rowIndex - 1}"][data-col="${colIndex}"]`
    );
    if (aboveCell) {
      ...
      aboveCell = document.querySelector(
        `.gridcell-optimal-neighbor[data-row="${
          rowIndex - 1
        }"][data-col="${colIndex}"]`
      );
      if (aboveCell) {
    ...
    let diagonalCell = document.querySelector(
      `.gridcell-neighbor[data-row="${rowIndex - 1}"][data-col="${
        colIndex - 1
      }"]`
    );
    if (diagonalCell) {
      ...
      diagonalCell = document.querySelector(
        `.gridcell-optimal-neighbor[data-row="${rowIndex - 1}"][data-col="${
          colIndex - 1
        }"]`
      );
      if (diagonalCell) {
  ...
  return (
    <div className="Home" id="Home">
      ...
      <div className="paragraph" style={{ fontSize: 15 }}>
        ...
        <a href="https://github.com/drdrsh/Needleman-Wunsch">
          https://github.com/drdrsh/Needleman-Wunsch
        </a>{" "}
        to support fitting alignments and local alignments. We found this tool
        helpful for visualizing how the global alignment recurrence is
        implemented, especially in how each value builds on those around it. By
        implementing fitting and local alignments, we can make this tool more
        useful for other types of alignments that we went over in class and
        handle a wider variety of alignment scenarios.
      </div>
      <div className="sequence-input">
        ...
        <div className="dropdown-div">
          ...
          <button className="action-button" onClick={handleComputeClick}>
            Compute
          </button>
        </div>
        <div className="grid">
            ...
            <tbody>
              <tr>
                ...
                {sequence2.split("").map((ele, index) => (
                  <td key={index} className="grid-label">
                    {ele}
                  </td>
                ))}
              </tr>
              {Object.keys(result).map((rowIndex) => (
                <tr key={rowIndex}>
                  <td
                    className={rowIndex === "0" ? "grid-empty" : "grid-label"}
                  >
                    {sequence1[rowIndex - 1]}
                  </td>
                  {Object.keys(result[rowIndex]).map((colIndex) => (
                    <td
                      ...
                      className={
                        path.some(
                          (pair) =>
                            String(pair[0]) === rowIndex &&
                            String(pair[1]) === colIndex
                        )
                          ? "gridcell-optimal"
                          : "gridcell"
                      }
                          ...
                          <table style={{ border: "1px solid black" }}>
                            <tr style={{ border: "1px solid black" }}>
                              <td style={{ border: "1px solid black" }}>
                                ...
                                {sequence1[rowIndex - 1] ===
                                sequence2[colIndex - 1]
                                  ? result[rowIndex - 1][colIndex - 1][0] +
                                    " + " +
                                    matchScore +
                                    " = " +
                                    result[rowIndex][colIndex][3] +
                                    " (Match between " +
                                    sequence1[rowIndex - 1] +
                                    " and " +
                                    sequence2[colIndex - 1] +
                              ...
                              >
                                ...
                                {result[rowIndex - 1][colIndex][0]} + {gapScore}{" "}
                                = {result[rowIndex][colIndex][2]}{" "}
                              </td>
                            </tr>
                            <tr style={{ border: "1px solid black" }}>
                              <td style={{ border: "1px solid black" }}>
                                ...
                                {result[rowIndex][colIndex - 1][0]} + {gapScore}{" "}
